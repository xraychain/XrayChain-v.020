...
xrc20burn
xrc20burnfrom
xrc20listtransactions
"""


class XrayXRC20Test(BitcoinTestFramework):
    ...
    def xrc20name_test(self):
        assert_equal(self.node.xrc20name(self.contract_address), "QRC TEST")

    def xrc20symbol_test(self):
        assert_equal(self.node.xrc20symbol(self.contract_address), "QTC")

    def xrc20totalsupply_test(self):
        assert_equal(self.node.xrc20totalsupply(self.contract_address), f"{10**65}.00000000")

    def xrc20decimals_test(self):
        assert_equal(self.node.xrc20decimals(self.contract_address), 8)
    ...
    def run_test(self):
        ...
        """
            ...
            /**
                XRC20Token Standard Token implementation
            */
            contract XRC20Token is SafeMath {
                string public constant standard = 'Token 0.1';
                uint8 public constant decimals = 8; // it's recommended to set decimals to 8 in XRAY
        ...
        self.xrc20name_test()
        self.xrc20symbol_test()
        self.xrc20totalsupply_test()
        self.xrc20decimals_test()
        assert_equal(self.node.xrc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")

        disconnect_nodes(self.node, 1)
        for n in self.nodes:
            assert_equal(n.getpeerinfo(), [])

        self.node.xrc20transfer(self.contract_address, self.creator, self.receiver, "0.00000001")
        self.node.generate(1)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.node.xrc20listtransactions(self.contract_address, self.creator, 0, 0), 
            [{
        ...
        assert_equal(self.node.xrc20listtransactions(self.contract_address, self.receiver, 0, 0), 
            [{
                'receiver': self.receiver, 
                'sender': self.creator,
                'amount': '0.00000001', 
        ...
        assert_equal(self.node.xrc20balanceof(self.contract_address, self.creator), f"{10**65-1}.99999999")
        assert_equal(float(self.node.xrc20balanceof(self.contract_address, self.receiver)), 0.00000001)


        # Double spend the xrc20 transaction to clear it from the mempool and reorg to clear the state
        ...
        assert_equal(self.node.xrc20listtransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.xrc20listtransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.xrc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(self.node.xrc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(float(self.reorg_node.xrc20balanceof(self.contract_address, self.receiver)), 0)
        assert_equal(float(self.node.xrc20balanceof(self.contract_address, self.receiver)), 0)
        ...
        self.node.xrc20transfer(self.contract_address, self.creator, self.receiver, f"{10**65}.00000000")
        self.node.generate(1)
        #sync_blocks(self.nodes)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.node.xrc20listtransactions(self.contract_address, self.receiver, 0, 0), 
        ...
        assert_raises_rpc_error(-1, "Not enough token balance", self.reorg_node.xrc20transfer, self.contract_address, self.receiver, self.creator, "0.1")

        # Make sure that sending from an address that we do not control fails gracefully
        assert_raises_rpc_error(-4, "Private key not available", self.reorg_node.xrc20transfer, self.contract_address, self.creator, self.receiver, "0.1")

        # Make sure that the allowance functionality works as intended
        self.approve_tx_receiver = self.node.getnewaddress()
        self.node.xrc20approve(self.contract_address, self.creator, self.receiver, "0.1")
        self.node.generatetoaddress(1, self.creator)
        sync_blocks(self.nodes)

        assert_equal(float(self.node.xrc20allowance(self.contract_address, self.creator, self.receiver)), 0.1)
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.xrc20transferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.10000001")

        self.reorg_node.xrc20transferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.01")
        self.sync_all()
        self.node.generatetoaddress(1, self.creator)
        sync_blocks(self.nodes)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.reorg_node.xrc20listtransactions(self.contract_address, self.approve_tx_receiver, 0, 0), [{
            'amount': '0.01000000',
            'blockHash': tip['hash'],
            'blockNumber': 2107,
            'blocktime': tip['time'],
        ...
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.xrc20transferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000001")

        self.reorg_node.xrc20transferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000000")
        ...
        assert_equal(float(self.node.xrc20allowance(self.contract_address, self.creator, self.receiver)), 0)
        assert_equal(self.reorg_node.xrc20balanceof(self.contract_address, self.creator), f"{10**65-1}.90000000")
        assert_equal(self.node.xrc20balanceof(self.contract_address, self.approve_tx_receiver), f"0.10000000")
