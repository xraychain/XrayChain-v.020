Gitian building
================
*Setup instructions for a Gitian build of RayChain using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Xray
...
- [Building RayChain](#building-xray-core)
...
Clone the git repositories for xray and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/raychainTR/xray --recursive
git clone https://github.com/raychainTR/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/xray/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the xray repository under 'Fetch and create inputs' to install sources which require
...
To build RayChain (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the xray repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xray/.git/
...
For example:
```bash
URL=https://github.com/laanwj/xray.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit xray=${COMMIT} --url xray=${URL} ../xray/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xray=${COMMIT} --url xray=${URL} ../xray/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xray=${COMMIT} --url xray=${URL} ../xray/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-xray:
./bin/gbuild --commit xray=${COMMIT},cpp-eth-xray=${ETHCOMMIT} --url xray=${URL},cpp-eth-xray=${ETHURL} ../xray/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the xray git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../xray/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/raychainTR/xray-detached-sigs.git

BTCPATH=/some/root/path/xray
SIGPATH=/some/root/path/xray-detached-sigs

./bin/gbuild --url xray=${BTCPATH},signature=${SIGPATH} ../xray/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xray-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xray-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/xray-osx-build.assert
```
