...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setXraySchedule(xrayDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = xrayDGP.getBlockSize(nHeight);
    minGasPrice = xrayDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = xrayDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    XrayTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractXrayTX resultConverter;
    if(!convert.extractionXrayTransactions(resultConverter)){
    ...
    for(XrayTransaction xrayTransaction : xrayTransactions){
        ...
        if(bceResult.usedGas + xrayTransaction.gas() > softBlockGasLimit){
        ...
        if(xrayTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, xrayTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            xrayDelegations.FilterDelegationEvents(events, *this);
            delegations_staker = xrayDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                xrayDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                xrayDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            xrayDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            xrayDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    XrayDelegation xrayDelegations;
    int32_t cacheHeight;
    ...
    void Update(interfaces::Chain::Lock& locked_chain, int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                xrayDelegations.FilterDelegationEvents(events, *this);
                pwallet->m_my_delegations = xrayDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    xrayDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
                    xrayDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                xrayDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(xrayDelegations.GetDelegation(address, delegation) && XrayDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "xraystake";
...
void StakeXrays(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
