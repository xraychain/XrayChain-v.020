...
const char * const BITCOIN_CONF_FILENAME = "xray.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Xray
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Xray
    // Mac: ~/Library/Application Support/Xray
    // Unix: ~/.xray
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Xray";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Xray";
#else
    // Unix
    return pathRet / ".xray";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("RayChain") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The RayChain Developers";
    }
