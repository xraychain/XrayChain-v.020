...
void XrayDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool XrayDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule XrayDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t XrayDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t XrayDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t XrayDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t XrayDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool XrayDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void XrayDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void XrayDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void XrayDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void XrayDGP::createParamsInstance(){
...
dev::Address XrayDGP::getAddressForBlock(unsigned int blockHeight){
...
void XrayDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void XrayDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void XrayDGP::parseStorageOneUint64(uint64_t& value){
...
void XrayDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule XrayDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
