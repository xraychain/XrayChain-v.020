...
UniValue transactionReceiptToJSON(const XrayTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination xraySenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(xraySenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&xraySenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToXrayAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToXrayAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
