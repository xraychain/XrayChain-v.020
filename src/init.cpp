 ...
 */
static const char* BITCOIN_PID_FILENAME = "xrayd.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("xray-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/xrayproject/xray>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateXray");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path xrayStateDir = GetDataDir() / "stateXray";
                bool fStatus = fs::exists(xrayStateDir);
                const std::string dirXray(xrayStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsXraystate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<XrayState>(new XrayState(dev::u256(0), XrayState::openDB(dirXray, hashDB, dev::WithExisting::Trust), dirXray, existsXraystate));
                dev::eth::ChainParams cp(chainparams.EVMGenesisInfo());
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(xrayStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setXraySchedule(xrayDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
