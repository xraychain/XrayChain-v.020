...
extern std::unique_ptr<XrayState> globalState;
...
inline void initState(){
    ...
    const std::string dirXray = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<XrayState>(new XrayState(dev::u256(0), XrayState::openDB(dirXray, hashDB, dev::WithExisting::Trust), dirXray + "/xrayDB", dev::eth::BaseState::Empty));
...
inline dev::Address createXrayAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline XrayTransaction createXrayTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    XrayTransaction txEth;
    if(recipient == dev::Address()){
        txEth = XrayTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = XrayTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<XrayTransaction> txs){
