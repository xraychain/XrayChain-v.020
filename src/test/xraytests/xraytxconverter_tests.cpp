...
void checkResult(bool isCreation, std::vector<XrayTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    XrayTxConverter converter(transaction, NULL);
    ExtractXrayTX xrayTx;
    BOOST_CHECK(converter.extractionXrayTransactions(xrayTx));
    std::vector<XrayTransaction> result = xrayTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(xraytxconverter_tests, TestingSetup)
