...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // xray
    uint256 hashUTXORoot; // xray
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // xray
        READWRITE(hashUTXORoot); // xray
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // xray
        hashUTXORoot.SetNull(); // xray
    ...
    virtual bool IsProofOfStake() const //xray
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //xray
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //xray
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // xray
        block.hashUTXORoot   = hashUTXORoot; // xray
