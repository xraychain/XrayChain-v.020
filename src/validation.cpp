...
#if defined(NDEBUG)
# error "Xray cannot be compiled without assertions."
#endif
...
std::unique_ptr<XrayState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = xrayDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = xrayDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        XrayTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractXrayTX resultConverter;
        if(!converter.extractionXrayTransactions(resultConverter)){
        ...
        std::vector<XrayTransaction> xrayTransactions = resultConverter.first;
        std::vector<EthTransactionParams> xrayETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(XrayTransaction xrayTransaction : xrayTransactions){
            sumGas += xrayTransaction.gas() * xrayTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, xrayTransaction.gasPrice());
            } else {
                txMinGasPrice = xrayTransaction.gasPrice();
            }
            VersionVM v = xrayTransaction.getVersion();
            if(v.format!=0)
            ...
            if(xrayTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(xrayTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(xrayTransaction.gas() > UINT32_MAX)
            ...
            if(v.rootVM!=0 && (uint64_t)xrayTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s XRAY additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // xray
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // xray
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    XrayTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = XrayTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = XrayTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<XrayTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path xrayDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(xrayDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(xrayDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(XrayTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, XrayTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool XrayTxConverter::extractionXrayTransactions(ExtractXrayTX& xraytx){
    ...
    std::vector<XrayTransaction> resultTX;
    ...
    xraytx = std::make_pair(resultTX, resultETP);
    return true;
}

bool XrayTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool XrayTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
XrayTransaction XrayTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    XrayTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = XrayTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = XrayTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t XrayTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // xray
    XrayDGP xrayDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setXraySchedule(xrayDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = xrayDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = xrayDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = xrayDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // xray
...
///////////////////////////////////////////////////////////////////////////////////////// xray
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            XrayTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractXrayTX resultConvertXrayTX;
            if(!convert.extractionXrayTransactions(resultConvertXrayTX)){
            ...
            if(!CheckMinGasPrice(resultConvertXrayTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertXrayTX.first, blockGasLimit, pindex->pprev);
            ...
            for(XrayTransaction& qtx : resultConvertXrayTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertXrayTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertXrayTX.first[k].from(),
                        resultConvertXrayTX.first[k].to(),
                        ...
                        resultConvertXrayTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(12000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // xray
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // xray
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // xray
            globalState->setRootUTXO(oldHashUTXORoot); // xray
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // xray
        uint32_t sizeBlockDGP = xrayDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Xray PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        XrayTxConverter convert(tx, NULL, NULL, contractflags);
