...
#define paternVersion "xray-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

XrayVersionChecker::XrayVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

XrayVersionChecker::~XrayVersionChecker()
{

}

bool XrayVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> XrayVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(XRAY_RELEASES)));
    QEventLoop event;
...
Version XrayVersionChecker::getMaxReleaseVersion()
{
