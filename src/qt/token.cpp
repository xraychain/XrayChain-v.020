...
Token::Token()
{
    ...
    lstMandatory.append(XrayToken::paramAddress());
    lstMandatory.append(XrayToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(XrayToken::paramSender());
    ...
    lstMandatory.append(XrayToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(XrayToken::paramAmount());
    lstOptional.append(XrayToken::paramGasLimit());
    lstOptional.append(XrayToken::paramGasPrice());
    lstOptional.append(XrayToken::paramSender());
    lstOptional.append(XrayToken::paramBroadcast());
    lstOptional.append(XrayToken::paramChangeToSender());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToXrayAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToXrayAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
