...
#include <qt/xrayversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    xrayVersionChecker = new XrayVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Xray address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and xray: URIs)"));
    ...
    XRCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/xrctoken"), tr("&XRC Tokens"), this);
    XRCTokenAction->setStatusTip(tr("XRC Tokens (send, receive or add Tokens in list)"));
    XRCTokenAction->setToolTip(XRCTokenAction->statusTip());
    XRCTokenAction->setCheckable(true);
    XRCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(XRCTokenAction);
    ...
    connect(XRCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(XRCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Xray addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Xray addresses"));
    ...
    openAction->setStatusTip(tr("Open a xray: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Xray command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(XRCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && xrayVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(XRAY_RELEASES, XRAY_RELEASES);
            QString message(tr("New version of Xray wallet is available on the Xray source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    XRCTokenAction->setEnabled(enabled);
...
{
    XRCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Xray network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
